...
- **`Service Ticket (Original)`** and **`Service Ticket (Copy)`** sheets — real, filled example (customer HAROUGE). Exact cell map (row/col), confirmed by direct read:
  - `B8` Customer, `E8` Ticket No, `B9` Field Name, `B10` Well No, `E10` MKN Supervisor, `B11` Rig Name, `E11` Start Job, `E12` End Job, `B12` Mileage (Km, one-way — `C16` formula does `=B12*2` for round trip), `B13` Base Location.
  - Line items start row 15 (headers) / 16 (first data row): `A` Item no., `B` Description, `C` Qty, `D` Unit, `E` Unit Cost, `F` Total Cost (`=C*E`).
  - Row 22 is a flat "DESERT/MARINE Surcharge" line (20% of items subtotal) — `D22`=0.2, `E22`=`SUM(F16:F21)`, `F22`=`E22*D22`.
  - Row 24: `B24` "Discount 60%", `E24`=`E22+F22`, `F24`=`-(E24*0.6)` — i.e. a standard discount is applied by default; check with the user whether this 60% is a fixed policy or per-ticket before hardcoding it into generation.
  - `A40`/`F40` grand total = `E24+F24`. Signature block rows 44/49.
