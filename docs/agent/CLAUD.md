...
| 2026-08-27 | **A ticket can be unwound: called off, withdrawn, or put back.** Three gaps reported from use. A technician who opened a ticket and then had the job cancelled had no honest exit — "Job Done" would have been a lie about work nobody did — so the record was carrying fictions. The office could not remove a ticket raised against the wrong customer. And nothing removed left an explanation behind. **Withdrawing is a soft delete on purpose:** `audit_log` cascades on `tickets`, so a hard delete would take the trail with it, and who withdrew a ticket and why is exactly what the person who raised it is owed. The row stays, the working lists stop showing it, the office can put it back. Migration 0034 carries the rules and 0035 the four capability keys — the second is not bookkeeping: `hasPermission()` treats a hydrated map as final, so a key only the client knows reads as **false** for every real signed-in user, and the control would simply never appear. All eight rules were probed against the live database with real impersonation before a line of client code: another technician cannot cancel your job, a technician cannot withdraw or restore anything, a cancelled job cannot be reopened by the man who cancelled it, and a withdrawn ticket accepts no edit but its own restoration. **One feature was not built:** the office closing a job somebody left open already existed, complete with a completeness gate a second version would have lacked — what it was missing was that it answered to a role rather than a capability, so `ticket.close_any` now gates the existing control instead. Also: a charge line can be typed from nothing, for a first job with a client who has no price list yet. |
| 2026-08-27 | **The password reset that could not reset a password (S3), and a leaked-password check the plan would not sell us (S4).** Rotating the seeded Admin password failed from the dashboard: the reset mail came back to the app and rendered the *login* screen, because nothing anywhere read the `type=recovery` fragment the link carries. There was no field to type a new password into, on any screen, for anyone. Now a recovery link opens a set-a-password screen, the login screen offers *Forgot your password?*, and **Settings carries a Password panel** — a technician who thinks somebody watched him type it should not have to ask the office. The fragment is read at module load, before the Supabase client exists, because creating that client parses the fragment and then clears it. Leaked-password protection turned out to be Pro-only, so on instruction it was built here instead: SHA-1 locally, send **five hex characters**, compare the returned suffixes on the device. The suite asserts the password never leaves — a test that only checked "breached passwords are refused" would pass for an implementation that posted the password to a stranger. An unreachable service allows the password and says so; a breach API in another country must not be why somebody at a wellhead cannot secure their account. One real bug caught by its own test: `sendRecovery` reached for `sb()` rather than `USE_CLOUD`, so a build pinned to `authMode: 'local'` — whose whole contract is that it never contacts the server — would have mailed a real person. |
| 2026-08-27 | **The ten parked Waha rows came back (S2, partly).** Restored under `-1` suffixes so they cannot collide with the liner hangers sharing their normalised range at 59,800–80,000 USD. They landed as `MKN-100-7xx-1`, not `MKN100-7xx-1`, because `trg_price_list_items_normalise` rewrites the prefix on insert — the house format, still distinct, so the outcome matches the intent. Validation of the real codes stays with Admin and Ops. |
| 2026-08-27 | **Tier 7 launched: `docs/FIELD-TEST.md`.** Ordered ahead of the Tier 6 design pass on the user's instruction, which costs nothing — the restyle would only repaint screens the field test uses. The pack fixes the go/no-go criteria **before** the data arrives, because deciding what counts as a failure after seeing the failures is how a bad build ships. Six blocking conditions, four count-based thresholds, and everything cosmetic routed to Tier 6 rather than allowed to hold up a decision. Writing the pre-flight checklist surfaced the one defect that would have invalidated the whole test: `BUILD` still read `night-shift.2 · 20 Aug 2026` after a week of shipping, so the login screen — `DEPLOY.md`'s single instrument for *which code is this phone running* — could not distinguish today's build from the 20 Aug one. Bumped to `field-test.1 · 27 Aug 2026` and read aloud by check 0.8. |
| 2026-08-27 | **Attachments became what they are for: the signed paperwork coming back (S22).** Built first as a general box — any file, any ticket, anyone who could see it. The actual job is collecting the two documents the client signs and stamps and returns, per ticket number, and only once the office has approved. Three rules now carry it, each enforced in the database and mirrored in the screen so a refusal is a sentence rather than a 403: nothing before approval; only the technician who did the job or the office; and every file says which of the two documents it is, because "pending" is unanswerable otherwise. The chase list — the half of the Tier 5 spec that was missed — lists what is still owed, to the office for every job and to a technician for his own. |
| 2026-08-27 | **The office stopped being told every technician was online (S21).** The in-progress presence badge read the viewer's own connectivity and printed it as "Online" next to somebody else's job, so the board showed a green dot on every live ticket whatever the field was doing — and the office uses that to decide whether it can raise someone. My own job still shows my live flag, because there it is the fact; anybody else's now shows when they were last in contact, derived from their position fixes and last sync, in those words. Nothing claims to know another person is online right now, because nothing can. Found by sweeping for unregistered stubs before closing Tier 5. |
| 2026-08-27 | **The job log and the audit trail stopped erasing each other (S19).** Ticket children synced by `replace` — delete every row for the ticket, insert the sender's list — so a technician syncing a job the office had also written on wiped what the office wrote. For the audit trail that is destruction of the record this project calls legally required, by a device that was only out of coverage. Log lines and audit entries now carry their own identity and sync row by row, so nobody's write deletes anybody's. Items, assets and crew stay wholesale on purpose: they have no identity beyond position and are built as a set by one person. |
| 2026-08-27 | **A sync conflict is now flagged rather than resolved by overwriting (S11 / B8).** The last device to reach the server used to win, silently. Every ticket row carries a version; a client sends the version its edit was made against, and an edit made against a version somebody has since replaced matches no row at all. The refusal goes to the set-aside pile with a sentence a person can act on, and is set aside on the first attempt rather than after five that cannot succeed. The trap this had to avoid: "no row matched" also describes a ticket raised offline that the server has never seen, and treating the two alike would throw away every job a technician starts out of coverage — so the two are told apart explicitly, and tested. |
| 2026-08-27 | **One fake Supabase client instead of three.** The stub was copied into each suite that needed it, and the copies drifted inside a single session — a fix to `insert()` landed in one while another went on failing for the reason the fix removed. Extracted to `app/cloudstub.js`. B-19.6 is about a fake that stops keeping up with the app; a fake that cannot keep up with itself is the same failure with more places to hide. |
| 2026-08-26 | **OneDrive and Google Drive dropped (S9).** The upload buttons were honest; the Connect step behind them was not — it flipped a stored flag and filled in a plausible account address, so Settings reported a connection that did not exist. With attachments in a private bucket behind RLS and expiring links, a copy in somebody's Drive is the weaker duplicate of an archive we now have. Removed, and the suite now guards the absence so the mock cannot return. |
| 2026-08-26 | **An oversized ticket's workbook adds up again (S10).** The customer form holds 24 item rows; a bigger ticket was cut to fit while the total cell still carried the whole job, so the visible lines did not sum to the printed figure. The last row is now a carry line worth exactly the difference — derived by subtraction from the total, so the two can never disagree — naming how many items it stands for and where they are itemised. |
| 2026-08-26 | **Attachments — the signed sheet coming back.** The first feature that deliberately does *not* work offline: the bytes go straight to a private bucket and the row straight to the table, because a 15MB photo cannot wait in a 5MB outbox. The control is disabled with no signal and says why, rather than accepting something it could never send. Metadata reaches the replica, so a technician down a hole still sees the sheet came back; the bytes never do. Files are served only through a 60-second signed URL (B-12.4), and a refused row deletes its own upload so the bucket never holds orphans. Closed **S16** on the way: notes and attachments now render on the technician's screen too. |
| 2026-08-26 | **A refusal can no longer wear a success's words.** `toast()` de-duplicated on the key alone, so a second banner under the same key kept the first one's text. Correct for the same button pressed twice; wrong the moment one key carries four outcomes, as `attach` does. The guard now compares the message (B-20.1). |
...
- **Price list context:** 2,610 rows generated, **2,600 live** (Zueitina 883, Waha 679, Harouge 421, Sirte 372, AGOCO 245). 110 rows have NULL unit_cost ("quoted separately"). App must display "Quoted Separately" not 0.00. Ten Waha code conflicts parked in backup table — do NOT invent numbers. **Diagnosed: see HANDOFF §8 row S2** — nine are Zueitina civil-works items alive as `MKN-0840`–`MKN-0848`, one (DEFOAMER - POWDER) is genuinely missing.
