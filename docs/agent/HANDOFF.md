...
| S19 | Only the ticket **header** is version-guarded. The children (`ticket_lines`, `ticket_items`, …) still go through `replace`, which is last-writer-wins per ticket. Not a silent overwrite of somebody's header any more, but two people adding job-log lines at once still resolve by replacement. | `index.html` | Tier 6 | — |
