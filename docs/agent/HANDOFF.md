...
| ~~S23~~ | ~~Withdrawing is a soft delete and there is no purge.~~ **Closed 2026-08-27 on the user's instruction: three months.** Migration 0036. Two things outlive the purge, and they are why it is not one line of `delete` in a cron job. **The number** — `tickets_ticket_number_key` is what stops one number reaching two clients, so deleting the row hands it back, and a client may be holding a signed sheet carrying it; paper does not expire after three months. A tombstone keeps every purged number and a trigger refuses to reissue one. **The fact it existed** — everything cascades off a ticket, so a purge is total; `purged_tickets` keeps the short account somebody would want years later (number, customer, who withdrew it, why, when). Nightly pg_cron at 02:00 UTC. Probed live: a four-month-old withdrawal goes, one from yesterday stays, its job log goes with it, the tombstone reads back in a sentence, and the number is refused for ever after. |
...
| ~~S12~~ | ~~`.mk-ticket-card` 3px stripe is dead code.~~ **Closed.** The class now owns the whole border and the markup sets only the stripe colour. Measured, not eyeballed: a 1px stripe and a 3px one look alike in a screenshot, which is how it survived. Pinned in `layout.test.js`. | CSS | ~~Tier 6~~ | — |
| S25 | Every text colour, tap target and page width is now measured by `a11y.test.js`, but only at **320px** and only on twelve representative screens. Wider widths and the remaining screens were clean when swept by hand (320/430/768/1280) and are not guarded. Widening the suite costs minutes per run; the trade was made deliberately, and 320 is the width everything fails at first. | tests | Tier 7 | — |
