...
| **Master workbook** | `master_export_rows` (view over approved `tickets`) | `exports` bucket, `export_runs` | `export.master`; rebuilt by pg_cron watermark, not a per-approval trigger | Account tab tile, signed download link, Edge Function `master-export` | ✅ Shipped and verified end to end — scheduler authorises with a single-use nonce (`export_nonces`), **no credential stored** | `excel` |
